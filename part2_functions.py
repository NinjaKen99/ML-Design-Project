...
def viterbi_algorithm(vocab, sentence, transition_params, emission_params):
    ...
    # Initialization
    for j in range(sentence_length):  # Iterate over the length of the sentence
    ...
    viterbi_matrix = np.array(viterbi_matrix)  # Convert the list to a NumPy array

    best_sequence = []
    curr_tag = viterbi_matrix.argmax(axis=0)[-1]

    for j in range(sentence_length - 1, -1, -1):
        best_sequence.insert(0, index_to_tags[curr_tag])
...
with ES_dev_p2_out as f_out: # change syntax to same as part 1 writing
    for i in modified_dev:
        sequence = viterbi_algorithm(emission_vocab,i,log_transition_param,log_emission_param)
