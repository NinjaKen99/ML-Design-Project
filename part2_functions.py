...
#I chatGPTed this, have not implimented yet.
...
#         viterbi_matrix[tag][0] = log(transition_params['START'][tag]) + log(emission_params[tag][sentence[0]])
    
#     # Recursive Step
#     for j in range(1, sentence_length):
#         for curr_tag in range(num_tags):
#             for prev_tag in range(num_tags):
#                 prob = viterbi_matrix[prev_tag][j - 1] + log(transition_params[prev_tag][curr_tag]) + log(emission_params[curr_tag][sentence[j]])
#                 if prob > viterbi_matrix[curr_tag][j]:
#                     viterbi_matrix[curr_tag][j] = prob
    
#     # Termination
#     for tag in range(num_tags):
#         viterbi_matrix[tag][sentence_length - 1] += log(transition_params[tag]['STOP'])
